...
params:
  ...
  select_aps:
    ...
    multiple: yes
  pathways_only_report:
    label: Create report for pathways team only?
    value: FALSE
  pathways_only_data:
...
if (any(grepl("Medical", select_aps))) {
  cat("-   **Medical consultations**: these are recorded with every visit by an athlete in person or electronically (e.g. telehealth)", sep = "\n\n")
} else {
  cat("")
}
if (any(grepl("Nurse", select_aps))) {
  cat("-   **Nursing consultations**: these are recorded with every visit by an athlete in person or electronically", sep = "\n\n")
} else {
  cat("")
}
if(any(grepl("Physiotherapy",select_aps))) {
  cat("-   **Physiotherapy treatment notes**: these are recorded with every visit by an athlete in person electronically", sep = "\n\n")
} else {
  cat("")
}
if (any(grepl("Psychology", select_aps))) {
  cat("-   **Psychology consultations**: these are recorded with every visit by an athlete in person or electronically", sep = "\n\n")
} else {
  cat("")
}
if (any(grepl("Nutrition", select_aps))) {
  cat("-   **Nutrition consultation**: these are recorded with every visit by an athlete in person or electronically", sep = "\n\n")
} else {
  cat("")
}
if (any(grepl("Life", select_aps))) {
  cat("-   **Performance Life communications**: these are recorded with every visit by an athlete in person or electronically", sep = "\n\n")
} else {
  cat("")
}
...
if (any(grepl("Medical", select_aps))) {
  cat("-   Medical data by Director of Performance Health", sep = "\n\n")
} else {
  cat("")
}
if (any(grepl("Nurse", select_aps))) {
  cat("-   Nursing data by Director of Performance Health", sep = "\n\n")
} else {
  cat("")
}
if(any(grepl("Physiotherapy",select_aps))) {
  cat("-   Physiotherapy data by Physiotherapy Lead and Director of Performance Health", sep = "\n\n")
} else {
  cat("")
}
if (any(grepl("Psychology", select_aps))) {
  cat("-   Psychology data by Psychology Head of Discipline", sep = "\n\n")
} else {
  cat("")
}
if (any(grepl("Nutrition", select_aps))) {
  cat("-   Nutrition data by Nutrition Head of Discipline", sep = "\n\n")
} else {
  cat("")
}
if (any(grepl("Life", select_aps))) {
  cat("-   Performance Life data by Performance Life Head of Discipline", sep = "\n\n")
} else {
  cat("")
}
...
```{r case_load, eval=!params$pathways_only_report, echo=FALSE, fig.height=7, fig.width=6, message=FALSE, warning=FALSE}
...
```{r aps_summaries_region, eval=!params$pathways_only_report, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r aps_summaries_sport, eval=!params$pathways_only_report, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r case_load_weekly, eval=!params$pathways_only_report, echo=FALSE, fig.height=3, fig.width=6, message=FALSE, warning=FALSE}
...
```{r case_load_sport, eval=!params$pathways_only_report, echo=FALSE, fig.height=2, fig.width=6, message=FALSE, warning=FALSE}
...
```{r case_load_region, eval=!params$pathways_only_report, echo=FALSE, fig.height=2, fig.width=6, message=FALSE, warning=FALSE}
...
```{r campaign_events, eval=!params$pathways_only_report, echo=FALSE, fig.width=6, message=FALSE, warning=FALSE}
...
```{r case_load_campaign, eval=!params$pathways_only_report, echo=FALSE, fig.height=8, fig.width=6, message=FALSE, warning=FALSE}
...
```{r aps_summaries_campaign, eval=!params$pathways_only_report, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r aps_summaries_sport_campaign, eval=!params$pathways_only_report, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r case_load_weekly_campaign, eval=!params$pathways_only_report, echo=FALSE, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
...
```{r case_load_sport_campaign, eval=!params$pathways_only_report, echo=FALSE, fig.height=2, fig.width=6, message=FALSE, warning=FALSE}

sports_campaign <- aps_campaign_weekly_caseload %>% 
  filter(games %in% c("Olympic Summer Games", "Paralympic Summer Games")) %>% 
  dplyr::pull(sport) %>% 
  unique() %>% 
  sort()
...
```{r unsupported_user_medical, eval=any(grepl("Medical",select_aps)), fig.width=6, message=FALSE, warning=FALSE}

filtered_data <- aps_consults %>% 
  filter(as.numeric(carding_status)>7) %>% 
  filter(aps_discipline == "Medical") 
...
filtered_data %>% 
  ...
  select(about, details, clinician_actual) %>%
  arrange(about, clinician_actual, details) %>% 
  flextable() %>% 
  set_caption(
              align_with_table = FALSE,
              word_stylename = "Table Caption",
  ...
  add_footer_lines(paste0(nrow(filtered_data), 
                          " total consultations."))  %>%
  theme_booktabs(bold_header = TRUE) %>%
  align(align = c("left"), part = "all") %>%
  merge_v( j = c("about", "details"), part = "body", combine = TRUE) %>% 
  valign(j = c(1:3), valign = "top", part = "all") %>%
  border_inner(border = hpsnz_flex_border) %>% 
  border_outer(border = hpsnz_flex_border) %>% 
...
```{r unsupported_user_nurse, eval=any(grepl("Nurse",select_aps)), fig.width=6, message=FALSE, warning=FALSE}

filtered_data <- aps_consults %>% 
  filter(as.numeric(carding_status)>7) %>% 
  filter(aps_discipline == "Nurse") 
...
filtered_data %>% 
  ...
  arrange(about, clinician_actual, details) %>% 
  flextable() %>% 
  set_caption(#caption = as_paragraph(as_b(paste0("Non-supported individuals receiving Nurse support"))),
              align_with_table = FALSE,
              word_stylename = "Table Caption",
              fp_p = fpp,
              autonum = TRUE) %>%
...
```{r unsupported_user_physiotherapy, eval=any(grepl("Physiotherapy",select_aps)), fig.width=6, message=FALSE, warning=FALSE}

filtered_data <- aps_consults %>% 
  filter(as.numeric(carding_status)>7) %>% 
  filter(aps_discipline == "Physiotherapy") 
...
filtered_data %>% 
  ...
  arrange(about, clinician_actual, details) %>% 
  flextable() %>% 
  set_caption(#caption = as_paragraph(as_b(paste0("Non-supported individuals receiving Physiotherapy support"))),
              align_with_table = FALSE,
              word_stylename = "Table Caption",
              fp_p = fpp,
              autonum = TRUE) %>%
...
```{r unsupported_user_psychology, eval=any(grepl("Psychology",select_aps)), fig.width=6, message=FALSE, warning=FALSE}

filtered_data <- aps_consults %>% 
  filter(as.numeric(carding_status)>7) %>% 
  filter(aps_discipline == "Psychology") 
...
filtered_data %>% 
  ...
  arrange(about, clinician_actual, details) %>% 
  flextable() %>% 
  set_caption(#caption = as_paragraph(as_b(paste0("Non-supported individuals receiving Psychology support"))),
              align_with_table = FALSE,
              word_stylename = "Table Caption",
              fp_p = fpp,
              autonum = TRUE) %>%
...
```{r unsupported_user_nutrition, eval=any(grepl("Nutrition",select_aps)), fig.width=6, message=FALSE, warning=FALSE}

filtered_data <- aps_consults %>% 
  filter(as.numeric(carding_status)>7) %>% 
  filter(aps_discipline == "Nutrition") 
...
filtered_data %>% 
  ...
  arrange(about, clinician_actual, details) %>% 
  flextable() %>% 
  set_caption(#caption = as_paragraph(as_b(paste0("Non-supported individuals receiving Nutrition support"))),
              align_with_table = FALSE,
              word_stylename = "Table Caption",
              fp_p = fpp,
              autonum = TRUE) %>%
...
```{r unsupported_user_life, eval=any(grepl("Life",select_aps)), fig.width=6, message=FALSE, warning=FALSE}

filtered_data <- aps_consults %>% 
  filter(as.numeric(carding_status)>7) %>% 
  filter(aps_discipline == "Life") 
...
filtered_data %>% 
  ...
  arrange(about, clinician_actual, details) %>% 
  flextable() %>% 
  set_caption(#caption = as_paragraph(as_b(paste0("Non-supported individuals receiving Life support"))),
              align_with_table = FALSE,
              word_stylename = "Table Caption",
              fp_p = fpp,
              autonum = TRUE) %>%
...
```{r unsupported_csv, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
